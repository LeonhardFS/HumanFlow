...
def wavg(val_col_name, wt_col_name):
    def inner(group):
        val = (group[val_col_name] * group[wt_col_name]
               ).sum() / group[wt_col_name].sum()
        return np.round(val)
...
def build_weighted_time_table(df_train, num_minutes=5):
    ...
    for i in xrange(56):
        ...
        df_train[wid] = 0.
        for day in xrange(1, num_days + 1):
            df_train.loc[df_train['day'] == day, wid] = 1. - \
                missing_per_day[day][sid]
    ...
    # Update: rounding the value is done in the wavg function, should be maybe
    # adjusted!
    col_name = 'S1'
    wid = 'w1'
    df_day_avg_values = pd.DataFrame(df_train[[col_name, 'day_time', wid]][df_train[
                                     col_name] != -1].groupby('day_time').apply(wavg(col_name, wid)), columns=[col_name])
    df_day_avg_values
    for i in xrange(1, 56):
        col_name = 'S' + str(i + 1)
        wid = 'w' + str(i + 1)
        print 'processing ' + col_name
        df_day_frame = pd.DataFrame(df_train[[col_name, 'day_time', wid]][df_train[
                                    col_name] != -1].groupby('day_time').apply(wavg(col_name, wid)), columns=[col_name])
